  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/budget", {
        ...
        body: JSON.stringify({ month: currentMonth, incomeTarget: parseFloat(incomeInput) || 0, expenseLimit: parseFloat(expenseInput) || 0 }),
      });
      const data = await res.json();
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-[10px]">
              ...
              {budget.incomeTarget > 0
                ? <span className={`font-bold ${incomeLeft >= 0 ? "text-emerald-500" : "text-rose-500"}`}>₹{Math.abs(incomeLeft).toLocaleString("en-IN")} {incomeLeft >= 0 ? "remaining" : "over target"}</span>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-[10px]">
              ...
              {budget.expenseLimit > 0
                ? <span className={`font-bold ${expenseLeft >= 0 ? "text-emerald-500" : "text-rose-500"}`}>₹{Math.abs(expenseLeft).toLocaleString("en-IN")} {expenseLeft >= 0 ? "left" : "over limit!"}</span>
